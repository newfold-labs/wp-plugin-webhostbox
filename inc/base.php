 ...
 * @return bool
 */
function webhostbox_has_plugin_install_date() {
	return ! empty( get_option( 'webhostbox_plugin_install_date', '' ) );
}
 ...
 * @return string
 */
function webhostbox_get_plugin_install_date() {
	return (string) get_option( 'webhostbox_plugin_install_date', gmdate( 'U' ) );
}
 ...
 */
function webhostbox_set_plugin_install_date( $value ) {
	update_option( 'webhostbox_plugin_install_date', $value, true );
}
 ...
 * @return int
 */
function webhostbox_get_days_since_plugin_install_date() {
	return absint( ( gmdate( 'U' ) - webhostbox_get_plugin_install_date() ) / DAY_IN_SECONDS );
}

/**
 * Basic setup
 */
function webhostbox_setup() {
	...
	if ( ! webhostbox_has_plugin_install_date() ) {
		$date = false;
		if ( ! empty( $install_date ) ) {
			$date = \DateTime::createFromFormat( 'M d, Y', $install_date );
		}
		webhostbox_set_plugin_install_date( $date ? $date->format( 'U' ) : gmdate( 'U' ) );
	}
}

add_action( 'admin_init', __NAMESPACE__ . '\\webhostbox_setup' );
 ...
 * @return int
 */
function webhostbox_install_date_filter( $install_date ) {
	return webhostbox_get_plugin_install_date();
}
add_filter( 'nfd_install_date_filter', __NAMESPACE__ . '\\webhostbox_install_date_filter' );
