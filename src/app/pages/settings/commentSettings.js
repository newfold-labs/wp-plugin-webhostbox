...
const OldPostsComments = ({ setError, notify }) => {
	...
	const disableCommentsNoticeTitle = () => {
		return disableCommentsOldPosts
			? __('Disabled old post comments', 'wp-plugin-webhostbox')
			: __('Enabled old post comments', 'wp-plugin-webhostbox');
	};

	const disableCommentsNoticeText = () => {
		return disableCommentsOldPosts
			? __('Comments on old posts are disabled.', 'wp-plugin-webhostbox')
			: __('Comments are allowed on old posts.', 'wp-plugin-webhostbox');
	};
...
const CloseCommentsDays = ({ setError, notify }) => {
	...
	const closeCommentsDaysNoticeTitle = () => {
		return (
			__('Comments setting saved ', 'wp-plugin-webhostbox')
		);
	};
	...
	const closeCommentsDaysNoticeText = () => {
		...
		return (
			__('Comments on posts are disabled after ', 'wp-plugin-webhostbox') +
			closeCommentsDays +
			_n(' day.', ' days.', parseInt(closeCommentsDays), 'wp-plugin-webhostbox')
		);
	};
	...
	const closeCommentsDaysLabelText = () => {
		...
		return (
			__('Close comments after ', 'wp-plugin-webhostbox') +
			closeCommentsDays +
			_n(' day.', ' days.', parseInt(closeCommentsDays), 'wp-plugin-webhostbox')
		);
	};
...
const CommentsPerPage = ({ setError, notify }) => {
	...
	const commentsPerPageNoticeTitle = () => {
		return __('Comments setting saved.', 'wp-plugin-webhostbox');
	};

	const commentsPerPageNoticeText = () => {
		//`Posts will display ${commentsPerPage} comments at a time.`
		return (
			__('Posts will display ', 'wp-plugin-webhostbox') +
			commentsPerPage +
			_n(
				' comment at a time.',
				' comments at a time.',
	...
	return (
		<SelectField
			...
			label={__('Display ', 'wp-plugin-webhostbox') + commentsPerPage +
				__(' comments per page.', 'wp-plugin-webhostbox')}
...
const CommentSettings = () => {
	...
	return (
		<Container.SettingsField
			title={__('Comments', 'wp-plugin-webhostbox')}
			description={__('Comments allow visitors to provide feedback and respond to your posts or pages.', 'wp-plugin-webhostbox')}
		>
