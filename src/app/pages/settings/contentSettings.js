...
const ContentRevisions = ({ setError, notify }) => {
	...
	const contentRevisionsNoticeTitle = () => {
		return (
			__('Post revision setting saved ', 'wp-plugin-webhostbox')
		);
	};

	const contentRevisionsNoticeText = () => {
		return (
			__('Posts will save  ', 'wp-plugin-webhostbox') +
			contentRevisions +
			_n(' revision.', ' revisions.', parseInt(contentRevisions), 'wp-plugin-webhostbox')
		);
	};

	const contentRevisionsDescriptionText = () => {
		return (
			__('Saving drafts and updating published content creates revisions. Make changes with confidence, knowing you can take ', 'wp-plugin-webhostbox') +
			contentRevisions +
			_n(' step back.', ' steps back.', parseInt(contentRevisions), 'wp-plugin-webhostbox')
		);
	};
	...
	return (
		<SelectField
			...
			label={__('Number of revisions posts can save ', 'wp-plugin-webhostbox')}
...
const EmptyTrash = ({ setError, notify }) => {
	...
	const emptyTrashNoticeTitle = () => {
		return (
			__('Trash setting saved ', 'wp-plugin-webhostbox')
		);
	};

	const emptyTrashNoticeText = () => {
		return (
			__('The trash will automatically empty every ', 'wp-plugin-webhostbox') +
			numTrashWeeks +
			_n( ' week.', ' weeks.', parseInt(numTrashWeeks), 'wp-plugin-webhostbox' )
		);
	};
	...
	return (
		<SelectField
			...
			label={__('Trash emptying frequency ', 'wp-plugin-webhostbox')}
			description={
			__('The trash will automatically empty every ', 'wp-plugin-webhostbox') +
			numTrashWeeks +
			 _n( ' week.', ' weeks.', parseInt(numTrashWeeks), 'wp-plugin-webhostbox' )
...
const ContentSettings = () => {
	...
	return (
		<Container.SettingsField
			title={__('Content Options', 'wp-plugin-webhostbox')}
			description={__('Controls for content revisions and how often to empty the trash.', 'wp-plugin-webhostbox')}
		>
