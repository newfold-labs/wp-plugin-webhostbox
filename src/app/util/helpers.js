...
export const webSettingsApiFetch = ( data, passError, thenCallback ) => {
	return apiFetch( {
		// path: 'webhostbox/v1/settings', //  can't use path bacause it breaks on temp domains
		url: NewfoldRuntime.createApiUrl( '/webhostbox/v1/settings' ),
		method: 'POST',
		data,
	} )
		.then( ( response ) => {
...
export const webPurgeCacheApiFetch = ( data, passError, thenCallback ) => {
	return apiFetch( {
		url: NewfoldRuntime.createApiUrl( '/webhostbox/v1/caching' ),
		method: 'DELETE',
		data,
	} )
		.then( ( response ) => {
...
export const addUtmParams = (url, params = {}) => {
	params.utm_source = `wp-admin/admin.php?page=webhostbox${window.location.hash}`;
	params.utm_medium = 'webhostbox_plugin';
